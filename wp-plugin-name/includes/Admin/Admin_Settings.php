...
final class Admin_Settings {
	...
	public function save_option_settings( array $data ): bool {
	...
	public function set_default_settings(): void {
		...
		if ( empty( $options ) ) {
			$this->save_option_settings( $this->default_options );
		}
	}
	...
	public function post_settings_form(): never {
		...
		if ( ! $this->is_security_check_valid() ) {
			wp_die(
				esc_html__( 'Sicherheitsprüfung fehlgeschlagen.', 'wp-plugin-name' ),
				esc_html__( 'Fehler', 'wp-plugin-name' )
			);
		}
		...
		if ( $this->save_option_settings( $options ) ) {
			...
			Admin_Notice::add_notice( esc_html__( 'Fehler beim Speichern der Einstellungen.', 'wp-plugin-name' ), 'error' );
		}
